...
- **Mentores temáticos (mentoría parametrizada por tema): ✅ IMPLEMENTADO (14/08).** Mentoría multitema **tema → sesión → avance**: el perfil pasa a ser un **registro de temas** (`perfil_mentor.json` v2: `temas: {slug: {nombre, objetivo_general, contexto, progreso, ultimo_avance_registrado, historial_sesiones}}` + `tema_activo`), con **migración idempotente v1→v2** y campos legacy de nivel superior conservados como **espejo del tema activo** (compatibilidad total con ids `mentor:*`, panel y tests existentes). API de temas en `perfil_mentor.py`: `listar_temas_mentoria()`, `cambiar_tema_activo(slug)`, `crear_tema(nombre, objetivo_general)`, `resolver_tema_mentoria(texto)` (auto-crea SOLO con señales explícitas "nuevo tema X / tema: X / empecemos con X"; mencionar un tema existente solo lo selecciona; persiste solo si hay cambio). `progreso_mentoria` ahora es **scoped por tema**: los eventos llevan campo `tema`, el avance se aplica al tema correcto (tema activo → espejo top-level; otro tema → `temas[slug]`, auto-creado sin activarse) y los recuerdos de la Bóveda viajan con `tema_relacionado` en metadatos. Persona de mentor genérica (ya no es "Mentor Tecnológico" hardcodeado): guía cualquier tema activo según `[PERFIL DEL TEMA ACTIVO]` que recibe en el prompt. **GUI consistente:** chip 🎯 de tema activo en el header (señal `__TEMA_ACTIVO__` desde `ia.py` en turnos mentor), selector + botón ➕ "Nuevo tema" en el sidebar (Capacidades), modal de creación (queda activo) y sección **"Temas de mentoría"** en el panel de memoria con olvido por tema (`mentor:tema:<slug>`). Tests: `tests/test_mentoria_multitema.py` (16: migración/espejo, API, olvido, sección panel, scoped por tema, preservación del registro en el reescritor de sesión).
...
**Suite de tests:** 447 passed (76 warnings preexistentes).
