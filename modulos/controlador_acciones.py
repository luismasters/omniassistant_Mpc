...
def procesar_acciones_ia(respuesta_ia, texto_usuario, ui_callback, modo_voz):
    ...
    in_code_block = False  # El escudo anti-código
    ...
    for linea in lineas:
        ...
        # Ignoramos líneas de los bloques grandes
        ...
        # LECTURA DE ARCHIVOS
        if linea_limpia.startswith("leer_archivo:"):
            idx = linea.lower().find("leer_archivo:") + 13
        ...
        # GITHUB
            ...
            datos_git = linea[linea.lower().find("github_reset:") + 13:].strip()
            ...
            datos_git = linea[linea.lower().find("git_comando:") + 12:].strip()
...
Responde ÚNICAMENTE con el Markdown final estructurado. No uses saludos, ni confirmaciones."""
        ...
        # COMANDOS DE SISTEMA (Mover Inteligente / Abrir / Cerrar)
        elif linea_limpia.startswith(("abrir:", "cerrar:", "mover:")):
            ...
            if cmd_idx != -1:
                ...
                cmd_extraido = linea[linea.lower().find(":", cmd_idx)+1:].replace('<', '').replace('>', '').strip()
                
                if verbo == "mover": 
                    cmd_extraido = cmd_extraido.replace("@ 1", "@ 2") if "@ 1" in cmd_extraido else cmd_extraido.replace("@ 2", "@ 1")
                    partes = cmd_extraido.split("@")
                    nombre_ventana_buscado = partes[0].strip().lower()
                    pantalla_destino = partes[1].strip() if len(partes) > 1 else "1"
                    
                    try:
                        num_pantalla = int(pantalla_destino)
                        # Llamamos directamente a la función de sistema.py SIN pasar por ejecutar_comando_sistema
                        threading.Thread(target=forzar_ventana_a_monitor, args=(nombre_ventana_buscado, num_pantalla), daemon=True).start()
                        reportes_acciones.append(f"Acción SO: Enviando orden para mover '{nombre_ventana_buscado}' al monitor {num_pantalla}")
                    except Exception as e:
                        reportes_acciones.append(f"Acción SO: Error al mover ventana: {e}")
                else:
